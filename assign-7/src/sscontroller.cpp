...
static void setAction(TokenScanner& scanner, SSModel& model) {
    ...
    //cout << scanner.getInput() << ' ' << scanner.getPosition() << endl;
...
static void clearContent(TokenScanner& scanner, SSModel& model) {
    if (scanner.hasMoreTokens())
        error("The clear command doesn't need any params");
    model.clearAllCells();
}
