...
enum ExpressionType { DOUBLE, TEXTSTRING, IDENTIFIER, COMPOUND, FORMULA };
...
class FormulaExp : public Expression {
public:
    /**
    * Constructor: FormulaExp
    * Usage: Expression *exp = new FormulaExp(funcName, cellr);
    * -------------------------------------------------------
    * The constructor initializes a new formula expression composed of
    * the function name and the range of cells.
    */

    /* Prototypes for the virtual methods overridden by this class */
    FormulaExp(const std::string func, const range cellr);
    ~FormulaExp();
    double eval(EvaluationContext& context) const;
    std::string toString() const;
    ExpressionType getType() const;
...
private:
    std::string _func;          /* function name like sum, average...*/
    range _cellr;               /* cell range like {A1, B2} */
};
